...
- `StreamEvent.thought_signature`, carried by the new `thought_signature` SSE
  ...
  text, and by the atomic `tool_use` event. The same value
  `ContentBlock.thought_signature` carries on a non-streaming response.
...
- `ContentBlock.thought_signature` is documented on **text** blocks as well as
  ...
  accepted it — this is the contract being stated, not a shape change.
- `provider_options` is documented as an open map, one JSON object per provider
  ...
  `HashMap<String, Value>`, so unnamed keys always rode through.
- `reasoning_effort` accepts `max` on every lane — the doc comment already said
  so and the gateway already validated it; this release adds the test that
  holds it.
